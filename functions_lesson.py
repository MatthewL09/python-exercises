...
-- the whole thing, within parenthesis is where we call the function
...
type(max([1, 2, 3]))
-- type(3) which is an integer 

# TODO: What will the output of the following code be? Why?
type(max)
-- max is a built_in_function_or_method
...
def increment(n):
    ...
    # function invocation is replaced with its return value
...
day = input('Please enter a day of the week and I will let you know if that is a weekend day or not: ')
...
add(y=3, x=4)   # order doesn't matter for keyword arguments
add(3, y=4)     # we can mix and match positional and keyword arguments
...
sayhello('Matthew')             # SAME
sayhello(name = 'Matthew')      # SAME
